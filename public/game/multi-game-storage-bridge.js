(function initializeMultiGameStorageBridge(root) {
  ...
  function createScopedStorageProxy(prototype, scoped) {
    ...
    return new Proxy(target, {
      get(proxyTarget, property, receiver) {
        if (typeof property === "symbol" || Reflect.has(proxyTarget, property)) {
      ...
      set(proxyTarget, property, value) {
        if (typeof property !== "string" || Reflect.has(proxyTarget, property)) {
          return false;
        }
      ...
      ownKeys() {
        return Array.from({ length: scoped.length }, (_, index) => scoped.key(index));
      },
      preventExtensions() {
        return false;
      },
  ...
  function install(win, scope) {
    ...
    const localStorageDescriptor = Object.getOwnPropertyDescriptor(win, "localStorage");
    ...
    const native = {
      ...
      getLength: descriptors.length?.get,
    };
    if (
      !prototype ||
      (localStorageDescriptor && !localStorageDescriptor.configurable) ||
      [...methodNames, "length"].some((name) => !descriptors[name]?.configurable) ||
    ...
    try {
      Object.defineProperties(prototype, {
        getItem: { configurable: true, writable: true, value(key) {
          return this === rawLocalStorage || this === scopedLocalStorage
            ? scoped.getItem(key) : native.getItem.call(this, key);
        } },
        setItem: { configurable: true, writable: true, value(key, value) {
          return this === rawLocalStorage || this === scopedLocalStorage
            ? scoped.setItem(key, value) : native.setItem.call(this, key, value);
        } },
        removeItem: { configurable: true, writable: true, value(key) {
          return this === rawLocalStorage || this === scopedLocalStorage
            ? scoped.removeItem(key) : native.removeItem.call(this, key);
        } },
        key: { configurable: true, writable: true, value(index) {
          return this === rawLocalStorage || this === scopedLocalStorage
            ? scoped.key(index) : native.key.call(this, index);
        } },
        clear: { configurable: true, writable: true, value() {
          return this === rawLocalStorage || this === scopedLocalStorage
            ? scoped.clear() : native.clear.call(this);
        } },
        length: { configurable: true, get() {
          return this === rawLocalStorage || this === scopedLocalStorage
            ? scoped.length : native.getLength.call(this);
        } },
      });
      ...
      try { Object.defineProperties(prototype, descriptors); } catch { /* fail closed */ }
      try {
        if (localStorageDescriptor) Object.defineProperty(win, "localStorage", localStorageDescriptor);
      } catch { /* fail closed */ }
      throw new Error("MultiGame storage bridge is not supported", { cause: error });
    }
  ...
  if (root.window === root && root.document) {
    ...
    const scope = params.getAll("scope").length === 1 ? params.get("scope") : "";
    const binId = params.getAll("bin_id").length === 1 ? params.get("bin_id") : "";
    try {
      if (!binId || !validateScope(scope)) throw new Error("Invalid launch parameters");
      const prefix = `multi-game:${scope}:`;
      if (
        root.localStorage.getItem(`${prefix}current_bin_id`) !== binId ||
        !root.localStorage.getItem(`${prefix}bin_data_${binId}`)
      ) {
        throw new Error("MultiGame account seed is unavailable");
      }
      install(root, scope);
      if (
        root.localStorage.getItem("current_bin_id") !== binId ||
        !root.localStorage.getItem(`bin_data_${binId}`)
      ) {
        throw new Error("MultiGame account seed is unavailable");
      }
      root.__MULTI_GAME_BRIDGE_READY__ = { scope, binId };
    } catch {
      root.__MULTI_GAME_BRIDGE_ERROR__ = { code: "bridge-install-failed" };
      const renderFatal = () => { root.document.body.textContent = "游戏账号隔离初始化失败"; };
      if (root.document.body) renderFatal();
      else root.document.addEventListener("DOMContentLoaded", renderFatal, { once: true });
      if (root.parent && root.parent !== root) {
        root.parent.postMessage(
          { channel: "multi-game", version: 1, type: "fatal", scope, code: "bridge-install-failed" },
