...
export function buildSnapshot(
  ...
  return {
    ...
    batchSettings: readJSON<Record<string, unknown>>(
      LS_KEYS.batchSettings,
      {},
    ),
...
function normalizeSnapshot(snap: AnyBackupSnapshot): BackupSnapshotV12 {
  ...
  const v12: BackupSnapshotV12 = {
    ...
    scheduledTasks: Array.isArray((snap as any).scheduledTasks)
      ...
      : [],
    batchSettings:
      (snap as any).batchSettings && typeof (snap as any).batchSettings === "object"
...
export function snapshotFilename(date = new Date()): string {
  ...
  return `xyzw-backup-${stamp}.json`;
}

export const LATEST_SNAPSHOT_FILENAME = "xyzw-backup-latest.json";
