...
金鱼杆补齐过程中会解析 `artifact_lottery` 返回的奖励数组；当奖励中出现赤羽道具 `13041` 时，按账号汇总钓获数量，并复用批量设置中的 WxPusher 配置发送 Markdown 表格通知。通知只在检测到赤羽时发送，未配置 WxPusher 时保留本地日志提示。
...
## 16. 一键梦境错误码 2600040 后继续购买（2026-05-18，本次提交）

### 16.1 问题

`batchmengjing` 原先把 `dungeon_selecthero` 和 `runDreamPurchaseForToken` 放在同一个 `try/catch` 中。若梦境前置指令返回服务端错误，例如 `2600040`，`sendMessageWithPromise` 会 reject，外层 catch 会直接将账号状态置为失败，导致梦境商品购买不会继续执行。

### 16.2 修复

- 单独包裹 `dungeon_selecthero` 调用。
- 解析 `服务器错误: <code>` 中的业务错误码。
- 当错误码为 `2600040` 时，仅记录 warning，并继续执行梦境购买。
- 其他错误码仍向外抛出，保持原有失败处理，避免真实异常被误吞。
...
| 日期 | 提交 | 变更摘要 |
|---|---|---|
| 2026-05-18 | 本次提交 | 一键梦境遇到 `2600040` 时继续执行梦境商品购买 |
